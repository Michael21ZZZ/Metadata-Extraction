...
def main(keyword_path, keyword_ref):
    keyword_file = pd.read_excel(keyword_path) # Modified for different disease, Input the keyword list
    # InputList should have 228 keywords (to do: input file should not contain ref keyword) It should be 229??
    kw_list = keyword_file['Words'].values.tolist()
    path = "./googletrends"
    ...
    normalised = normalise(combined, n_file = num_iteration - 1, key_ref="diabetes causes", col='date')
...
if __name__ == '__main__':
    ...
    keyword_ref = 'diabetes causes' # use the first keyword for comparison in each iteration. A
    main(keyword_path, keyword_ref)